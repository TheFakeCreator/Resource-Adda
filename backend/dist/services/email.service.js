...
const sendVerificationEmail = async (to, token) => {
    try {
        ...
        if (!process.env.RESEND_API_KEY) {
            ...
            console.log(`Link: ${process.env.FRONTEND_URLS?.split(",")[0] || "http://localhost:3000"}/verify-email?token=${token}`);
            console.log("==========================================");
            return true;
        }
...
const sendPasswordResetEmail = async (to, token) => {
    try {
        if (!process.env.RESEND_API_KEY) {
            ...
            console.log(`To: ${to}`);
            console.log(`Token: ${token}`);
            console.log(`Link: ${process.env.FRONTEND_URLS?.split(",")[0] || "http://localhost:3000"}/reset-password?token=${token}`);
            console.log("==========================================");
            return true;
        }
        const frontendUrl = process.env.FRONTEND_URLS?.split(",")[0] || "http://localhost:3000";
