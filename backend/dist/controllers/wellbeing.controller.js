...
exports.toggleReaction = exports.addComment = exports.getComments = exports.createPost = exports.getPosts = void 0;
...
const getPosts = async (req, res) => {
    try {
        ...
        const total = await WellbeingPost_1.default.countDocuments(filter);
        const posts = await WellbeingPost_1.default.find(filter)
    ...
    try {
        ...
        if (req.files && Array.isArray(req.files)) {
            for (const file of req.files) {
                const url = await (0, upload_1.uploadToCloudinary)(file.buffer, "wellbeing");
                mediaUrls.push(url);
            }
        }
        let parsedTags = [];
        ...
        const newPost = new WellbeingPost_1.default({
            ...
            author: req.user._id,
            isAnonymous: req.body.isAnonymous !== "false" && req.body.isAnonymous !== false,
            tags: parsedTags,
            mediaUrls,
            reactions: [],
        });
...
const getComments = async (req, res) => {
    try {
        const { id } = req.params;
        const comments = await WellbeingComment_1.default.find({
            post: id,
            status: "approved",
        })
            .populate("author", "name avatarUrl branch semester")
