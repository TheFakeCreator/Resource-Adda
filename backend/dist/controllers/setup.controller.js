...
exports.getPublicSettings = exports.configureSystem = exports.getSetupStatus = void 0;
...
const getSetupStatus = async (req, res) => {
    try {
        ...
        if (userCount === 0) {
            res.status(200).json({ isSetupComplete: false, message: 'Super admin registration pending' });
            return;
        }
        if (!settings || !settings.isSetupComplete) {
            res.status(200).json({ isSetupComplete: false, message: 'System configuration pending' });
            return;
        }
...
const configureSystem = async (req, res) => {
    try {
        ...
        await settings.save();
        res.status(200).json({ message: 'System configuration saved successfully', settings });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
    }
...
const getPublicSettings = async (req, res) => {
    try {
        const settings = await SystemSettings_1.default.findOne();
        if (!settings) {
            res.status(200).json({ taglineLanguage: 'hindi', instituteName: 'Resource-Adda', allowedEmailPatterns: [] });
            return;
        }
        res.status(200).json({
