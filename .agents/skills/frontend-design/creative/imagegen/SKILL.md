...
- If the user says “a hero image for a landing page”, you may add _layout/composition constraints_ that are implied by that use (e.g., “generous negative space on the right for headline text”).
...
- **`references/cli.md`**: how to _run_ image generation/edits/batches via `scripts/image_gen.py` (commands, flags, recipes).
