...
const DUMMY_RESOURCE: DocumentDetails = {
  ...
  description:
    "These are my complete handwritten notes for Engg Math 1. It covers Matrices, Calculus, and Vector Spaces. I've highlighted the important theorems that usually come in the end semester exams.",
  fileUrl:
...
const DUMMY_REVIEWS: Review[] = [
  {
    ...
    rating: 5,
    comment:
      "These notes are an absolute lifesaver! Thank you so much for sharing.",
    upvotes: 4,
    downvotes: 0,
    upvotedBy: ["u3", "u4"],
    downvotedBy: [],
...
export default function ResourceDetailPage() {
    ...
    const fetchDetails = async () => {
      ...
      try {
        const [docRes, revRes] = await Promise.all([
          api.get(`/resources/documents/${documentId}`),
          api.get(`/resources/documents/${documentId}/reviews`),
        ]);
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      await api.post(
        `/resources/documents/${documentId}/reviews`,
  ...
  const handleVote = async (
    ...
    try {
      await api.post(
        `/resources/reviews/${reviewId}/vote`,
        { voteType },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      ...
      setReviews((prev) =>
        prev.map((rev) => {
          if (rev._id === reviewId) {
            ...
            const newUpvotes = [...rev.upvotedBy].filter(
              (id) => id !== user?._id,
            );
            const newDownvotes = [...rev.downvotedBy].filter(
              (id) => id !== user?._id,
            );
  ...
  const typeConfig =
    RESOURCE_TYPE_COLORS[document.type.toLowerCase()] ||
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        className={`w-8 h-8 ${star <= newRating ? "fill-amber-500 text-amber-500" : "text-slate-300 dark:text-slate-700"}`}
                      />
                    </button>
                  ))}
                </div>
                  ...
                  >
                    <CardContent className="p-5">
                          ...
                          <div>
                            ...
                            <div className="flex items-center text-amber-500">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <Star
                                  key={star}
                                  className={`w-3 h-3 ${star <= review.rating ? "fill-current" : "text-slate-300 dark:text-slate-700"}`}
                                />
                              ))}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <ThumbsUp
                            className={`w-3.5 h-3.5 mr-1.5 ${review.upvotedBy.includes(user?._id || "") ? "fill-current" : ""}`}
                          />
                          {review.upvotes || 0}
                        </Button>
                        <Button
                        ...
                        >
                          <ThumbsDown
                            className={`w-3.5 h-3.5 mr-1.5 ${review.downvotedBy.includes(user?._id || "") ? "fill-current" : ""}`}
                          />
                          {review.downvotes || 0}
                        </Button>
                      </div>
