...
export default function AdminPagesEditor() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/pages/${selectedSlug}`,
        { title, content },
  ...
  if (isLoading || !user) return <div className="p-8 flex items-center justify-center min-h-screen"><Loader2 className="animate-spin h-8 w-8 text-primary" /></div>;

  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Markdown Pages Editor</h1>
            <p className="text-muted-foreground">Manage static content pages globally.</p>
          </div>
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card className="flex flex-col h-[75vh] shadow-lg">
              <CardHeader className="bg-muted/50 border-b pb-4">
                ...
                <CardDescription>Write your content in standard Markdown.</CardDescription>
                
                <div className="mt-4 space-y-2">
                  <Label htmlFor="title" className="text-foreground font-bold">Page Title</Label>
                  <Input
                    id="title"
                    value={title}
            ...
            <Card className="flex flex-col h-[75vh] shadow-lg overflow-hidden">
              ...
              <CardContent className="p-8 overflow-y-auto bg-background flex-1">
                <h1 className="text-4xl font-extrabold text-foreground mb-8 text-center">{title || 'Untitled Page'}</h1>
                <div className="markdown-body text-foreground space-y-6 leading-relaxed">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      th: ({node, ...props}) => <th className="border border-border bg-muted px-4 py-2 font-semibold text-left" {...props} />,
                      ...
                      code(props) {
                        ...
                        return match ? (
                          ...
                          <code {...rest} className="bg-muted px-1.5 py-0.5 rounded text-sm font-mono text-primary">
                            {children}
                          </code>
                        )
                      }
