...
export default function AdminSettingsPage() {
  ...
  }, [isLoading, isAuthenticated, user, router, instituteName, taglineLanguage]);
    ...
    try {
      await axios.post(
        'http://localhost:5000/api/setup/configure',
        {
          ...
          allowedEmailPatterns: formData.allowedEmailPatterns.split(',').map(s => s.trim()).filter(Boolean),
      ...
      <Card className="w-full border shadow-sm">
        <CardHeader className="space-y-1 bg-muted/50 rounded-t-xl pb-6 border-b">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-sm font-semibold tracking-wider text-primary uppercase">Super Admin Only</h2>
          </div>
        </CardHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="instituteName" className="text-foreground font-bold">Institute Name</Label>
                    <Input
                      id="instituteName"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="allowedEmailPatterns" className="text-foreground font-bold">Allowed Email Domains (Comma separated)</Label>
                    <Input
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Re-enter domains to update them. Empty values will be ignored.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="taglineLanguage" className="text-foreground font-bold">Hero Tagline Language</Label>
                    <select
                      id="taglineLanguage"
                      value={formData.taglineLanguage}
                      onChange={(e) => setFormData({ ...formData, taglineLanguage: e.target.value })}
                    ...
                    >
                      ...
                      <option value="english">English (Professional/Alternative)</option>
                    </select>
                    <p className="text-xs text-muted-foreground mt-1">
