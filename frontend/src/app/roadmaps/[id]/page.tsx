...
import { Target, Clock, ShieldCheck, Ghost, User as UserIcon, BookOpen, AlertCircle, Link as LinkIcon, Star, MessageSquare } from 'lucide-react';
...
const MOCK_ROADMAP = {
  ...
  introNotes: "I used this exact roadmap to secure an SDE role at a top product company. The key is to build projects alongside learning rather than getting stuck in tutorial hell. Good luck!",
  globalPrerequisites: [
  ...
  steps: [
    {
      ...
      description: "Before diving into React, ensure your JS fundamentals are rock solid. Focus on Promises, async/await, closures, and the Event Loop.",
      prerequisites: ["Basic JS Variables and Loops"],
      resources: [
        { title: "Namaste JavaScript (Youtube)", url: "#", type: "Video" },
        { title: "MDN Web Docs - Closures", url: "#", type: "Article" }
      ]
    },
    {
      _id: "s2",
      title: "React.js Foundation",
      description: "Learn component-based architecture, hooks (useState, useEffect), and state management. Build a simple Todo App.",
      prerequisites: ["Advanced JavaScript"],
      resources: [
        { title: "React Official Documentation", url: "#", type: "Article" },
        { title: "Frontend Masters - React", url: "#", type: "Course" }
      ]
    },
    {
      _id: "s3",
      title: "Node.js & Express Basics",
      description: "Understand the backend. Learn how to create a server, handle routes, and build RESTful APIs using Express.",
    ...
    },
    {
      ...
      description: "Connect your Express app to a NoSQL database. Learn schemas, models, and CRUD operations.",
      prerequisites: ["Node.js & Express"],
      resources: [
        { title: "Mongoose Docs", url: "#", type: "Article" }
      ]
    },
    {
      _id: "s5",
      title: "Full Stack Integration (MERN)",
      description: "Bring it all together! Connect your React frontend to your Express backend. Build a full-stack E-commerce app.",
      prerequisites: ["React.js", "MongoDB", "Express"],
      resources: [
        { title: "Full Stack E-Commerce Tutorial", url: "#", type: "Video" }
      ]
    }
  ],
  isOfficial: true,
...
export default function RoadmapDetailPage() {
  ...
  useEffect(() => {
    if (mounted) {
      localStorage.setItem(`roadmap_progress_${id}`, JSON.stringify(completedSteps));
    }
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <section className="bg-muted/30 border-b border-border pt-12 pb-16 relative overflow-hidden">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl relative z-10">
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <Badge variant="outline" className="px-3 py-1 bg-slate-500/10 text-slate-600 border-slate-500/20 dark:text-slate-400">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4 leading-tight">{roadmap.title}</h1>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl flex items-center justify-between gap-4">
          <div className="flex-1">
            <div className="flex justify-between mb-2 text-sm font-medium">
              ...
              <span className="text-primary">{progressPercentage}% ({completedSteps.length}/{roadmap.steps.length})</span>
            </div>
            <Progress value={progressPercentage} className="h-2.5" />
          </div>
          <Button variant="outline" size="sm" className="hidden sm:flex" onClick={() => setCompletedSteps([])}>
            Reset
          </Button>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl py-12">
        ...
        {roadmap.globalPrerequisites && roadmap.globalPrerequisites.length > 0 && (
        ...
        <div className="relative pl-4 md:pl-8">
          ...
          <div className="space-y-12">
            {roadmap.steps.map((step, index) => {
              ...
              return (
                <div key={step._id} className="relative z-10 flex gap-6 md:gap-8 group">
                  ...
                  <div className="mt-1 flex-shrink-0">
                    <div className={`relative w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                      isCompleted ? 'bg-primary border-primary text-primary-foreground shadow-lg shadow-primary/30' : 'bg-background border-muted-foreground/30 text-muted-foreground hover:border-primary'
                    }`}>
                      <Checkbox 
                        id={`step-${index}`} 
                        checked={isCompleted}
                      ...
                      />
                      <span className="font-bold pointer-events-none">{index + 1}</span>
                    </div>
                  </div>

                  {/* Step Card */}
                  <Card className={`flex-1 transition-all duration-300 ${
                    isCompleted ? 'border-primary/50 bg-primary/5 shadow-md' : 'border-border bg-card opacity-90 hover:opacity-100 hover:shadow-md'
                  }`}>
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <CardTitle className={`text-xl md:text-2xl transition-colors ${isCompleted ? 'text-primary' : ''}`}>
                          {step.title}
                        </CardTitle>
                      </div>
                      ...
                      {step.prerequisites && step.prerequisites.length > 0 && (
                        <div className="bg-muted/50 rounded-lg p-4">
                          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center">
                            <BookOpen className="w-3.5 h-3.5 mr-1.5" /> Required Knowledge
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {step.prerequisites.map((req, i) => (
                              <Badge key={i} variant="secondary" className="bg-background text-xs">{req}</Badge>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {step.resources && step.resources.length > 0 && (
                        <div>
                          <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center">
                            <LinkIcon className="w-3.5 h-3.5 mr-1.5" /> Recommended Resources
                          </p>
        ...
        {progressPercentage === 100 && (
          <div ref={completionRef} className="mt-16 bg-gradient-to-br from-emerald-500/20 to-teal-500/10 border border-emerald-500/30 rounded-2xl p-8 text-center animate-in fade-in zoom-in duration-500">
            ...
            <h2 className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mb-2">Congratulations! 🎉</h2>
            <p className="text-muted-foreground max-w-lg mx-auto">
              You've completed all the steps in this roadmap. Don't forget to practice building projects and applying what you've learned!
            </p>
          </div>
        )}
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center text-amber-500 font-bold">
              ...
              {roadmap.averageRating} <span className="text-muted-foreground text-sm font-normal ml-1">({roadmap.totalRatings} ratings)</span>
            </div>
          </div>
