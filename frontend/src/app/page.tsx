...
import { ArrowRight, BookOpen, GraduationCap, UploadCloud, Map, Target, Heart, Sparkles, MessageSquare, ShieldCheck, Users, Briefcase } from 'lucide-react';
import { useConfigStore } from '@/store/useConfigStore';

const STATS = [
  { label: "Verified Resources", value: "1,200+", icon: <BookOpen className="w-5 h-5 text-blue-500" /> },
  { label: "Interview Experiences", value: "85+", icon: <Briefcase className="w-5 h-5 text-indigo-500" /> },
  { label: "Active Roadmaps", value: "24", icon: <Map className="w-5 h-5 text-emerald-500" /> },
  { label: "Community Members", value: "3,500+", icon: <Users className="w-5 h-5 text-amber-500" /> },
];
...
const TESTIMONIALS = [
  {
    ...
    content: "Resource Adda's interview experiences and roadmaps were instrumental in my placement journey. It's exactly what I needed during my final year.",
    avatar: "https://ui-avatars.com/api/?name=Rahul+Verma&background=random"
  },
  {
    name: "Sneha Patel",
    role: "3rd Year, IT",
    content: "The featured section with stress-free notes saved me during the end-sems. I didn't have to scroll through endless PDFs to find what matters.",
    avatar: "https://ui-avatars.com/api/?name=Sneha+Patel&background=random"
  },
  {
    name: "Aman Gupta",
    role: "2nd Year, CS",
    content: "I love the MERN stack roadmap. It gave me a clear step-by-step path instead of getting lost in tutorial hell.",
  ...
  return (
    <div className="flex-1 bg-background flex flex-col items-center w-full relative overflow-hidden">
      ...
      <section className="w-full relative flex flex-col items-center justify-center min-h-[calc(100vh-5rem)] px-4 py-10 md:py-12 text-center">
        ...
        <Badge variant="outline" className="mb-8 border-primary/30 text-primary bg-primary/5 px-4 py-1.5 rounded-full font-medium shadow-sm flex items-center gap-2">
        ...
        <div className="max-w-5xl relative z-10 w-full flex flex-col items-center">
            ...
            <div className="relative inline-block rotate-[-2deg] mb-8 md:mb-10 mr-8 md:mr-16">
              ...
              <div className="bg-slate-900/90 backdrop-blur-md text-white p-6 md:p-10 rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,0.8)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.8)] border-4 border-slate-900 dark:border-white transition-transform hover:scale-105 duration-300">
                <p 
                  ...
                  style={{ fontFamily: 'var(--font-marker)', letterSpacing: '2px' }}
                >
          ...
          <p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl leading-relaxed">
            Everything you need to ace your exams, learn modern skills, and crack top placements. Curated by the community, for the community.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <Link href="/register">
              <Button size="lg" className="w-full sm:w-auto h-16 px-10 text-xl rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,0.8)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.8)] border-2 border-foreground hover:translate-y-1 hover:shadow-none transition-all font-bold group">
                Join the Community <ArrowRight className="ml-2 h-6 w-6 group-hover:translate-x-1 transition-transform" />
              </Button>
            </Link>
            <Link href="/explore">
              <Button variant="outline" size="lg" className="w-full sm:w-auto h-16 px-10 text-xl rounded-2xl bg-background shadow-[4px_4px_0px_0px_rgba(0,0,0,0.8)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.8)] border-2 border-foreground hover:translate-y-1 hover:shadow-none transition-all font-bold text-foreground">
                Explore Library
              </Button>
            </Link>
          </div>
      ...
      <section className="w-full border-y border-border bg-muted/20 py-10">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-border/50">
            {STATS.map((stat, idx) => (
              <div key={idx} className={`flex flex-col items-center justify-center text-center ${idx % 2 !== 0 ? 'pl-8' : ''}`}>
      ...
      <section className="w-full py-24 px-4 container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">Everything in <span className="text-primary">One Place</span></h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Stop asking seniors for drive links. We've organized everything you need into beautiful, easy-to-digest formats.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[320px]">
          ...
          <Link href="/explore" className="md:col-span-2 md:row-span-2 group">
            <Card className="h-full relative overflow-hidden border-2 border-border hover:border-blue-500/50 transition-all duration-300 bg-gradient-to-br from-background to-blue-500/5">
              ...
              <CardContent className="p-10 flex flex-col h-full justify-end relative z-10">
                ...
                <p className="text-lg text-muted-foreground max-w-md">Access a massive collection of verified notes, previous year papers, and assignments categorized cleanly by branch and semester.</p>
              </CardContent>
            </Card>
          </Link>
            ...
            <Card className="h-full relative overflow-hidden border-2 border-border hover:border-rose-500/50 transition-all duration-300 bg-gradient-to-br from-background to-rose-500/5">
              <CardContent className="p-8 flex flex-col h-full justify-between">
                <div className="flex justify-between items-start">
                  ...
                  <Badge variant="outline" className="border-rose-500/30 text-rose-500 bg-rose-500/10">Priority</Badge>
                </div>
                <div>
                  <h3 className="text-xl font-bold mb-2">Student Wellbeing</h3>
                  <p className="text-sm text-muted-foreground">Curated, stress-free study materials designed to help you breathe easy during exams.</p>
                </div>
              </CardContent>
            </Card>
          </Link>
          ...
          <Link href="/roadmaps" className="md:col-span-1 md:row-span-1 group">
            <Card className="h-full relative overflow-hidden border-2 border-border hover:border-emerald-500/50 transition-all duration-300 bg-gradient-to-br from-background to-emerald-500/5">
              <CardContent className="p-8 flex flex-col h-full justify-between">
                ...
                <div>
                  <h3 className="text-xl font-bold mb-2">Interactive Roadmaps</h3>
                  <p className="text-sm text-muted-foreground">Track your progress step-by-step through skill and academic guides.</p>
                </div>
              </CardContent>
            </Card>
          </Link>
          ...
          <Link href="/placements" className="md:col-span-3 md:row-span-1 group">
            <Card className="h-full relative overflow-hidden border-2 border-border hover:border-indigo-500/50 transition-all duration-300 bg-gradient-to-r from-background to-indigo-500/10">
              ...
              <CardContent className="p-10 flex flex-col md:flex-row items-center justify-between h-full gap-8">
                <div className="flex-1">
                  ...
                  <h3 className="text-3xl font-bold mb-3">Interview Experiences</h3>
                  <p className="text-lg text-muted-foreground max-w-xl">Read round-by-round breakdowns, CTC details, and exact preparation strategies from seniors who cracked FAANG and top product companies.</p>
                </div>
      ...
      <section className="w-full py-24 bg-muted/30 border-y border-border">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <Badge variant="outline" className="mb-4 bg-primary/10 text-primary border-primary/20">Wall of Love</Badge>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">Loved by Students</h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Don't just take our word for it. Here is what the community has to say.</p>
          </div>
      ...
      <section className="w-full py-32 px-4 relative overflow-hidden bg-slate-950 dark:bg-slate-900 text-white">
        ...
        <div className="container mx-auto max-w-4xl text-center relative z-10">
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-6 text-white">Ready to Ace Your Semester?</h2>
          <p className="text-xl text-slate-300 mb-10 max-w-2xl mx-auto">Join thousands of students sharing notes, strategies, and cracking placements together.</p>
          
          <Link href="/register">
            <Button size="lg" className="h-16 px-12 text-xl rounded-full bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-105 transition-all shadow-xl shadow-primary/20 font-bold">
              Create Free Account
            </Button>
          </Link>
          <p className="text-sm text-slate-400 mt-6">Takes less than 30 seconds. No spam ever.</p>
        </div>
      </section>
